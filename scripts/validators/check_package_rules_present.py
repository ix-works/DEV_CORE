"""
check_package_rules_present.py — Her ERP/<PKG>/ paketinde `.rules.md` var mı kontrol eder.

Kullanım:
    python scripts/validators/check_package_rules_present.py
    python scripts/validators/check_package_rules_present.py --erp-root ERP
...
def find_packages(erp_root: Path) -> list[Path]:
    """ERP/<MODULE>/<PKG>/ pattern'i — modül seviyesi altındaki paketleri döner."""
...
def main() -> int:
    ...
    parser.add_argument("--erp-root", default="ERP", help="ERP root dizini (varsayılan: ERP)")
