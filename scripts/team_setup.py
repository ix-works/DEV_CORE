...
def junctions(proje: Path, overlay_onayli: bool = False) -> bool:
    """4 junction + zorunlu `rules` kopyası (D25: her biri TEK TEK raporlanır — kopuk agents/skills
    SESSİZ semptom verir). Junction: core · .claude/agents · .claude/skills · .claude/commands.
    `.claude/rules` (L1b, 2026-07-10) 2026-09-12'den beri junction DEĞİL — aşağıdaki ZORUNLU
    OVERLAY ile gerçek dizindir ve ayrı bir `overlay .claude/rules` satırı basar (Q243/Q286).
...
def smoke(proje: Path) -> bool:
    ...
    ortam = mcp_calisma_env(CORE_ROOT, proje)[1]   # denetim ortamının kaynağı (.mcp.json / şablon)
    if ok:
