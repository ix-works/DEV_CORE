...
def properties_metni(b: bytes) -> str:
    ...
    t = satir_sonu_normalize(b).decode("utf-8", errors="replace")
    t = _U_KACIS.sub(lambda m: m.group(1) + chr(int(m.group(2), 16)), t)
    return t.encode("utf-16", "surrogatepass").decode("utf-16", errors="replace")
...
def dosya_kiyasla(rel: str, canli: bytes, yerel: bytes) -> tuple[str, str, str, str]:
    ...
    if Path(rel).suffix.lower() == ".properties":
        pc, py = properties_metni(canli), properties_metni(yerel)
        if pc == py:
...
def zip_coz(zip_bayt: bytes) -> dict[str, bytes]:
    try:
        ...
        raise Olculemedi(f"zip çözülemedi ({len(zip_bayt)}B): {e}")
...
def yaz(kaynak: dict[str, bytes], hedef: Path) -> None:
    ...
    for rel, b in kaynak.items():
        ...
        if kok not in p.resolve().parents:   # zip içi `../` yolu hedef dışına yazamaz
            raise Olculemedi(f"zip girdisi hedef dizin dışını gösteriyor: {rel!r}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if a.zip:
            ...
            zip_bayt, ust = zip_indir(bsp, read_conn())
        ...
        print(f"[OLCULEMEDI] {e}", file=sys.stderr)
        print(kapsam_beyani("— (indirme BAŞARISIZ)", "KOŞMADI", False))
        return 2
    ...
    if a.eslik:
        try:
            ...
            print(f"[OLCULEMEDI] eşlik: {e}", file=sys.stderr)
            print(kapsam_beyani(zip_kaynagi, f"ÖLÇÜLEMEDİ — {e}", False))
            return 2
        ...
        if sapma and sinif != ESLIK_YOK:
            ...
            sinif = ESLIK_YOK
        if sinif == ESLIK_YOK:
            print("     DUR: --out ve --karsilastir KOŞMADI; kaynağı kullanıcıdan iste.")
        eslik_durumu = f"{sinif} (build ↔ aynı zip'teki {PRELOAD}; deploy_ui.preload_karsilastir)"
        if sinif == ESLIK_YOK:
