...
| **U1 JIT-recall enjeksiyonu** | mcp-sap-docs (FTS5+BM25 fikri) + claude-mem index-deseni + resmî hooks additionalContext | UserPromptSubmit'te `.tmp/recall-index.json`'a karşı skorlama → eşik üstü top-K ders-İNDEKS-SATIRI `additionalContext` olarak enjekte edilir (tam metin değil) | `scripts/hooks/recall_inject.py` + `settings.template.json` UserPromptSubmit | ✅ **AKTİF** (radar 2026-08-28'de canlı ölçüldü — önceki `⏳ ADAY` etiketi **BAYATTI**). ⚠ **Mekanizma kaynaktan FARKLI:** FTS5/BM25 **DEĞİL** (dosyada 0 eşleşme) — **ağırlıklı token-kesişimi** (`ESIK=5`, başlık-token'ı 3 puan ⇒ tek eşleşme yetmez; `<40` karakterlik prompt'ta sıfır çıktı; LLM yok; fail-open). Türkçe-BM25 zayıflığı bu tasarımla **konu dışı kaldı**. ⚠ `settings.json`'da **TEK kayıt** olmalı — 2026-08-11'e kadar çift kayıtlıydı (token israfı) |
...
| **SAP RESMÎ ADT MCP server** | SAPSE.adt-vscode (help.sap.com) | VS Code ADT eklentisi içinde resmi MCP (RAP generator, activate, debugger; v1.0 source-read YOK) — custom server'ımızın uzun-vadeli emeklilik sorusu | — | ⚪ **İZLE (çeyreklik):** source-read/debugger olgunlaşınca göç analizi |
