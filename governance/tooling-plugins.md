...
| **abaplint** | arc-1 deseni (⚠ repo taşındı: `marianfoo/arc-1` → **`arc-mcp/arc-1`**, ölçüldü 2026-08-28) + `@abaplint/cli` | tuned ABAP lint (class_push gap) | `scripts/abaplint/abaplint.json` + `check_abaplint.py` (`ABAPLINT_PIN` satır 40) | ✅ aktif (reviewer WARNING) — ⚠ **pin 2.120.5, npm latest 2.120.38 = 33 sürüm / 91 commit geride** (ölçüldü 2026-08-28). Blog/CHANGELOG'da duyuru YOK ama commit günlüğünde ilgili düzeltmeler VAR (`CDS syntax checks #4223` · `cds_naming namespaced prefix #4244` · `cloud_types SMBC/TOBJ/G4BA #4231` · `CLASS_CONSTRUCTOR public section #4217`) ⇒ *duyuru yok ≠ değişiklik yok*. **Bump ADAY** — önce/sonra aynı class üzerinde yan-yana koşum + `tests/fixtures/abaplint_failopen/run.py` (21/21) tekrarı şart |
