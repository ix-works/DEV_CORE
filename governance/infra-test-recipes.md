...
- **`source` DALI (2026-08-29).** SessionStart girdisindeki `source` (`startup|resume|clear|compact|fork`) gövdeyi ikiye ayırır.
  ...
  Korpus: `python tests/fixtures/session_start_compact_dali/run.py` → **22/22**.
  ⛔ **V3 TABANI = DONDURULMUŞ KOPYA** (`taban_session_start_1746040.py` = fix-öncesi main). İlk sürüm tabanı `git show HEAD:` ile alıyordu; commit atılınca HEAD fix'i içerdi ve CI (PR #179) V3'te düştü — merge sonrası main SÜREKLİ kırmızı kalırdı. SHA'ya `git show` da CI'da çalışmaz (sığ klon). Kopyada `STATIK_COMPACT` görülürse V3 OLCULEMEDI der (PASS değil). **ÜÇ mutasyon** (hiçbiri diğerini kapsamaz): `--mutasyon-dalsiz` → **15/22** (düşen V1·V1b·V2·V8·V8b·V9·V10) · `--mutasyon-capasiz` → **19/22** (V2·V8·V8b) · `--mutasyon-state` → **19/22** (V2·**V7**·V8). FP çapaları (V3 bayt-eşlik · V4/V4b · V5/V6/V6b fail-safe · V11 · V12 · V13 · V14) ÜÇ mutasyonda da **AYAKTA**.
