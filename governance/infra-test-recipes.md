...
- `python tests/run_battery.py fetch_ui_source --kardes ui_dogrulama_satir_sonu_kacis --precommit` → taban 54/54 + 26 kip DÜŞER.
...
- ⛔ SİLİNEMEZ: **B4** (yorum farkı gizlenmez) · **B6/B7/B7b** (manifest ölçülen-liste) · **A6/B11** (ikili normalize
  ...
  **C7** (zip-slip → rc 2 + boş hedef) · **C8-C12** (bozuk girdi/G-Ç → rc 2, çökme değil) · **C14/C15** (`.conn_adt`
  yok/boş → rc 2) · **B12-B15** (tip-duyarlı manifest, katı `.properties`) · **Z0** (çökme = FAIL).
...
- ⛔ Resolver filtresi `FUGR/FF`'dir. `FUNC` / `FUNC/FF` VAR OLAN FM için de 0 döner (canlı ölçüldü) — `--mutasyon-func-filtresi` bunu çiviler, R9a emülatörü kalibre eder.
  ⚠ **Düzeltme 2026-09-13 (Q306①, B53):** bu 0 `sap_client.search_objects` katmanının ölçümüdür. Ham quickSearch `FUNC`/`FUNC/FF` FM'i `FUGR/FF` tipiyle DÖNDÜRÜR; 0'ı istemci tip süzgeci (`FUNC≠FUGR`) üretir. Resolver için kural aynen geçerli; MCP `adt_search_objects` artık FM takma adını `FUGR/FF`'ye çevirir.
...
- Canlı teyit (YAZMA — kullanıcı onayı + adt-gateway, en dar kapsam): ① bekleyen FM'i olan bir FUGR'da `adt_activate(object_type="fugr")` → `ok:true` · `activation_verified:true` · bağımsız `adt_inactive_objects`'te grup ve FM yok. ⚠ 2. faz GÖVDESİ ve `hukum_sebep` MCP `client_log`'unda GÖRÜNMEZ (`SAPClient.activate_object` bool döner, yalnız `[OK]`/`[FAIL]` + hata metni basar; `_debug` yalnız `ADT_SAP_DEBUG=1` dosyasına ref SAYISINI yazar). Gövde ekseni için lib doğrudan çağrılır: `SAPADTClient().activate_object(...)` sözlüğünde `aktivasyon_hukmu` · `hukum_sebep` · `fugr_faz2` (`ff` listesi) + istek sarmalayıcısıyla POST params/gövde ② aynı turda zaten aktif bir DDLS'i yeniden aktive et → başarı, tek POST, worklist GET yok (kontrol grubu).
...
```
python tests/fixtures/sorgu_araclari_durustlugu/run.py                  # 38 vektör, SAP gerektirmez, exit 0
python tests/fixtures/sorgu_araclari_durustlugu/run.py --taban eab0180  # eski üretim kodu (bellekte) → 11/38, exit 1 = karşıtlık VAR
python tests/run_battery.py sorgu_araclari_durustlugu --kardes sorgu_basarisizligi_gorunur fm_okuma_where_used aktivasyon_govde_hukmu b0_secim --precommit
```

- **Harness:** beş üretim modülü (`sap_adt_lib` · `sap_client` · `query` · `worklist_audit` · `where_used`) diskten okunur, kipin metin yaması uygulanır, `exec` edilir; repoya yazılmaz. Çapa TAM 1 kez geçmezse `[KURULAMADI]` exit 2 (KAÇTI değil). `--taban <ref>` beşini `git show <ref>:`'den yükler; taban fix'i zaten taşıyorsa `[DOGRULANAMADI]` exit 2.
- ⚠ `worklist_audit` ve `where_used` import anında (win32) `sys.stdout`/`stderr`'i aynı buffer üzerine yeniden sarar; ikinci sarma birincisini sahipsiz bırakır, GC buffer'ı kapatır ⇒ **çıktısız rc=1** (`I/O operation on closed file`). Fixture asıl akışları geri koyar ve modülün sarmalayıcısını `detach()` eder. Bu iki modülü import eden yeni harness aynısını yapmalı.
- ⛔ **`truncated` KESİNDİR, tahmin değil:** araç `row_limit+1` satır ister (`_sonda_limiti`), fazlası gelirse `true` + sonda atılır (`_sondayi_kes`). Tam `row_limit` satırlık sonuç kırpık SAYILMAZ (S4b · S8b; `--mutasyon-q304-kirpma-esik` eski `>=` tahminini geri koyar). `totalRows`'tan türetme YOK (S6: aggregate'de alttaki satır sayısı; `--mutasyon-q304-kirpma-tahmin`). Emülatör `rowNumber`'a UYMALI (canlı: limit N → N satır); uymayan sahte sunucu sondayı ölçemez.
- ⛔ **TADIR "kırpık" = yalnız SQL yanıtının `truncated`'ı.** TADIR'da satırı olmayan ad kırpık DEĞİLDİR ve bugün `tadir_deleted:false` alır (T5 eski kodda da yeşil = davranış değişmedi; Q adayı, bilerek dokunulmadı). T6 gerçek `adt_sql_query` yolundan geçer (stub yok).
- ⛔ Başarısız ya da kırpık parça YALNIZ kendi adlarını `null` yapar (T2: 25 ad, 1 parça 400 → 5 null, 20 ölçülü, `ok:false`, `count` YOK). `--mutasyon-q305-sessiz-parca` başarısız parçayı "soruldu" sayar.
- ⭐ **Q306① katman ayrımı:** sahte sıfır istemci süzgecindedir; eşleme MCP katmanında yapılır, `sap_client.search_objects` süzgeci DEĞİŞMEZ (U5 resolver · B45 R9a o katmanı ölçer). Sunucunun GERÇEK sıfırı (`FUGR` → 0) uyarı üretmez (U6). U2'deki `CDS` takma adı SENTETİKTİR (görünürlük dalını ölçmek için), bir SAP iddiası değildir.
- ⭐ **Q306② canlı ağaç biçimi** (V1): obje düğümleri (ENHO/XHB, CLAS/OC) + tipsiz kullanım satırı + zincirli `DEVC/K` ataları. Yalnız paket düğümü (V2/V6) canlıda hiç görülmedi — sentetik, fail-closed dalını ölçer.
- ⭐ **Q310 kablolama** X4'te nöbetçiyle ölçülür: kanonik `aktivasyon_worklist_ayristir` monkeypatch edilince İKİ yol da nöbetçi listeyi döndürmeli. Kopya ayrıştırıcı geri gelirse X4 düşer.
...
## B61 — Sözdizimi CLI'si ölçülemediyi `has syntax errors` diye basmaz: `olculemedi_sebebi` + DDIC HTTP≠200/404 ve istisna `valid:None` (Q317; B51 + B56'nın CLI tarafı)

```
python tests/fixtures/aktivasyon_govde_hukmu/run.py                  # 117 vektör, SAP gerektirmez, exit 0
python tests/fixtures/aktivasyon_govde_hukmu/run.py --mutasyon       # "MUTASYON OZETI: 37/37", exit 0
python tests/fixtures/aktivasyon_govde_hukmu/run.py --taban cb777ae  # Q317 öncesi üretim kodu → 111/117, exit 0 = karşıtlık VAR
python tests/run_battery.py aktivasyon_govde_hukmu --kardes push_onkontrol_olculemedi sorgu_basarisizligi_gorunur veri_yetki_guardlari aktivasyon_sahte_ok mcp_sahte_sonuc_uclusu --precommit
```

- **Harness:** J25 GERÇEK yoldan kurulur. Sahte sunucu HTTP 400 döner, lib `SAPADTError` fırlatır, `SAPClient.syntax_check` bunu yutup `{'valid': False, 'error': …}` döndürür. Vektör önce sözlüğün biçimini doğrular (errors YOK, `error` VAR), sonra aynı örneği CLI'ye verir. J27 SENTETİKTİR (bugün hiçbir üretim yolu hem `errors` hem `error` döndürmüyor): sınırı çivilemek için vardır, gerçek hata kaydı olduğunda `error` metni yüzünden UNVERIFIED'a düşülmez. K12/K14'te `requests.get` `ConnectionError` fırlatır; K bölümünün `finally`'si onu geri alır.
- `--taban cb777ae` beklenen: düşenler **tam olarak** J25 · K8 · K9 · K12 · K13 · K14. K9 eski kodda yalnız başlık yüzünden düşer (`SAP did not run the check`). KONTROL satırlarından biri (J24 · J26 · J27 · K7 · K10 · K11) düşerse vektör ya da harness bozuktur, karşıtlık değildir.
- ⚠ Çapa tuzağı (B51/B56 ile aynı): UNVERIFIED çıktısı *"does NOT mean the source has syntax errors"* cümlesini taşır. Ayırt edici dizgeler `[FAIL] SYNTAX CHECK FAILED` ve `NOT MEASURED`'dir. K11 404'ün başlığını **tam satırla** çapalar: `[FAIL] SYNTAX CHECK FAILED - <ad> has syntax errors`.
- ⛔ SİLİNMEZ: J24 · J26 · J27 · K11 (errors DOLU False `[FAIL]` kalır; M29/M30'un tek ayırt edicileri) · K7 + M28 (404 False) · K9'daki `SAP did not run` yokluğu (M35). K8'in çapası Q317 ile BİLEREK çevrildi (False → None `ddic_http_500`); geri çevirmek M32'yi kaçırır.
- Mutasyon satırında **`MUTASYON OZETI: 37/37`** satırını ayrıca oku (B50 uyarısı aynen geçerli: koşucu hepsi yakalanınca exit 0, batarya `AYIRDI(rc=0)` gösterir).
- ⚠ MCP yayılımı yok: `syntax_check.py` yalnız CLI'dir, MCP süreci onu import etmez. `sap_client.py` bu turda değişmedi.
- Canlı teyit (SALT-OKUR, proje kökü `.conn_adt` → `set_explicit_working_dir(<proje kökü>)`, çıktı maskeli): `GET /sap/bc/adt/activation/inactiveobjects` içinde `DTEL`/`DOMA`/`TABL`/`TTYP` girdisi olan bir obje varsa parametresiz GET ile `?version=active` ve `?version=inactive` GET'lerinin kök `adtcore:version` değerleri kaydedilir (3 GET). Parametresiz GET aktif gövde döndürürse `check_ddic_object` bekleyen değişikliği ölçmeden `True` der; bu durum ayrı bir Q adayıdır. 2026-09-13 (VPN sonrası, 2 GET): kanonik Accept `application/vnd.sap.adt.inactivectsobjects.v1+xml` → kök `ioc:inactiveObjects`, `aktivasyon_worklist_ayristir` → `CLAS/OC` 2 · `CLAS/OM` 2 · DDIC **0** ⇒ ÖLÇÜLEMEDİ, canlı vaka yok (vaka ÜRETİLMEZ). ⚠ Accept'i yalnız `application/xml` verirsen aynı uç `adtcore:objectReferences` döner (OM girdileri yok); `ioc` ayrıştırıcısı 0 bulur ve sahte "liste boş" hükmü çıkar — sondada lib'in Accept'ini kullan. LOCK/PUT/POST YOK.

## B62 — Issue #274–#278 (Q335–Q339): MCP import denetimi · ix_doctor K3 `repo_mode` · K1 1e PowerShell politikası · session_start gerilik sayısı

```
python tests/fixtures/mcp_import_denetimi/run.py            # 21/21 (~15 sn; ağ/SAP yok)
python tests/fixtures/ix_doctor_repo_mode/run.py            # 10/10
python tests/fixtures/ix_doctor_ps_politika/run.py          # 13/13 (Windows'ta C3 gerçek kayıt defteri)
python tests/fixtures/session_start_origin_geriligi/run.py  # 8/8
python tests/run_battery.py mcp_import_denetimi --kardes ix_doctor_repo_mode ix_doctor_ps_politika session_start_origin_geriligi team_setup_hook_kablolama ix_doctor_kablolama session_start_compact_dali d7_drift_imzasi core_fiziksel_kopya adtget_yokluk_kaniti b0_secim --precommit
```

- **Eski-kod karşıtlığı (tek seferlik, git'e bağlı DEĞİL):** eski dosyaları bir dizine çıkar (`git show <sha>:scripts/team_setup.py` vb.) ve ver: `mcp_import_denetimi --kaynak-dizini <DIR>` (team_setup.py · ix_doctor.py · requirements.txt) · `ix_doctor_repo_mode --kaynak <ix_doctor.py>` · `ix_doctor_ps_politika --kaynak <ix_doctor.py>` · `session_start_origin_geriligi --kaynak <session_start.py>`. `ab68ba9` ile ölçülen: 15/21 · 5/10 · 0/13 · 5/8. ⚠ `mcp_import_denetimi` tabanında M8 Issue senaryosunu ÜRETMEZ (eski smoke PYTHONPATH'i EZİP yardımcıyı çağırmaz ⇒ makinedeki gerçek `mcp` import edilir); M6/M7/M11 5b2 satırı olmadığı için, M12 eski smoke `.mcp.json`'u okumadığı için düşer. Issue'nun gerçek kanıtı en alttaki venv reçetesidir.
- ⛔ **`ix_doctor_repo_mode` R4·R5·R6·R7·R9 eski kodda da GEÇMELİ** — bu, ⚠GEVŞETME'nin (Q336) yalnız `local`/`none`'a sınırlı olduğunun kanıtıdır. Eski kodda bunlardan biri düşüyorsa gevşetme sınırı kaymıştır.
- ⭐ **Hedef GERÇEK `__file__` ile koşulur, repo ağacına bayt yazılmaz:** `mcp_import_denetimi/_kosucu.py` ve `ix_doctor_repo_mode/_senaryo.py` kaynak METNİNİ (gerçek/mutant/taban) `compile(metin, <gerçek yol>)` ile exec eder ⇒ `CORE_ROOT = parents[1]` doğru çözülür. Mutantı `scripts/` altına kardeş dosya olarak YAZMA.
- ⭐ **Sahte `mcp` paketi + GERÇEK zincir:** CI mcp kurmaz. Korpus `mcp` v1 (asgari FastMCP) / v2 (gerçek 2.2.0 shim mesaj kalıbı + `[SAHTE-V2]` çapası) paketlerini yardımcının **YALNIZ-TEST `ek_yol`** parametresiyle "kurulu paket" yerine koyar (`.mcp.json` PYTHONPATH'inin ARKASINA); CLI vektörlerinde `_kosucu.py` bunu `_IX_TEST_EK_YOL`'dan sarar (üretim kodu bu adı okumaz). `mcp_servers.sap_adt.server → _app.py` GERÇEKTİR (sandbox proje `core` bağı → depo).
- ⛔ **Denetim ortamı = `.mcp.json` ortamı — kullanıcı PYTHONPATH'i EZİLİR.** CLI vektörlerinde kullanıcı PYTHONPATH'ine BİLEREK ters sürüm konur; `--mutasyon-one-ekle` (ilk sürümün reddedilen "öne ekle, kullanıcı yolunu koru" davranışı) M4a·M4b·M4c·M6·M7·M8·M9'u düşürür (ölçüldü). Yeni vektörde "kullanıcı yolu" ile "kurulu paket"i aynı sürüm YAPMA — ezme ölçülmemiş olur.
- ⛔ **Şablona düşüş YALNIZ `.mcp.json` HİÇ YOKKEN (bug-gate-2 M1).** Dosya var ama bozuksa (geçersiz JSON · `sap-adt` yok · `env` nesne değil) ⇒ ÖLÇÜLEMEDİ; M4f/M4g/M4h "kurulu" mcp'yi **v1** yapar ⇒ şablona düşülseydi `import-ok` dönerdi (ayırt edici). `--mutasyon-bozuk-sablona` (ilk sürüm) M4f·M4g·M4h'yi düşürür. `env` anahtarı YOKSA `{}` = kullanıcı ortamı (M4i; `--mutasyon-envsiz-sablona` düşürür).
- ⛔ **Çare metni son satırdan (bug-gate-2 L1):** `No module named 'mcp_servers…'` ⇒ `.mcp.json`/`core` bağı çaresi, pip DEĞİL; `mcp` hatası ⇒ pip. M11 (ix_doctor, `core` bağı yok) + M12 (team_setup, `.mcp.json` PYTHONPATH olmayan dizin) + M6/M8 (pip dalı); `--mutasyon-tek-care` M11·M12'yi düşürür.
