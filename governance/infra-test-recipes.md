...
```
python tests/fixtures/mcp_import_denetimi/run.py            # 12/12 (~5 sn; ağ/SAP yok)
...
- ⭐ **Sahte `mcp` paketi + GERÇEK zincir:** CI mcp kurmaz. Korpus `mcp` v1 (asgari FastMCP) / v2 (gerçek 2.2.0 shim mesaj kalıbı + `[SAHTE-V2]` çapası) paketlerini PYTHONPATH'e koyar; `mcp_servers.sap_adt.server → _app.py` GERÇEKTİR. Bu, yardımcının PYTHONPATH'i **öne eklemesine** bağlıdır — `--mutasyon-ezme` bunu çiviler (bu makinede gerçek 1.27.1 kuruluyken M6/M8 PASS'a döner = düşer).
