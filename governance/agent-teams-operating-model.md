...
Uzun-yaşayan background ajanlarla (özellikle standing gateway) çalışırken `idle_notification` gecikmesi + task re-trigger'ı **sahte "blocked/stale" gürültüsü** üretir. Beş kural bunu kaynağında keser:
...
- **E — Kararı DEĞİŞTİREN mesaj gönderdiysen, teslimi RAPORDAN DEĞİL KODDAN doğrula (2026-09-10; sınıfın 3. vakası).** Ajanın raporu **kendi içinde tutarlı** olduğu hâlde bayat olabilir: mesaj ulaşmadan önce yazılmış olabilir, ya da `idle_notification` özeti bir önceki turu taşıyor (§4A-A). ⛔ Rapor eski kararı anlatıyor diye **"mesajım düşmüş / ajan almamış" DEME** — bu iddia ölçülmeden kurulamaz ve **üç vakanın üçünde de yanlış çıktı** (ajan talimatı almış ve uygulamış; bayat olan rapordu). Doğru hüküm tek yerden kurulur: değişen kararın **kodda karşılığını ölç** (içerik-çapası `grep` + `md5`), ölçüm ne diyorsa o. ⚠ Kullanıcıya *"ajan mesajı almadı"* diye bildirmek **ölçülmemiş bir suçlamadır**; bunu yapma. Ajan tarafındaki karşılığı = spawn-brief §8 **TALİMAT-DEĞİŞİKLİĞİ TEYİDİ** (ajanın kendi cümlesiyle özeti, liderin ölçümünün yerine geçmez — **tamamlayıcısıdır**). Bkz. `feedback_sendmessage-gorevi-sessizce-islenmemis-olabilir`.
> Not: gerçek blocker'lar (DDLS shell reçetesi, generic-tip aktivasyon-fail, pseudo-comment yerleşimi) bu gürültüden AYRIDIR — onlardaki gidiş-geliş normal SAP-geliştirme iterasyonudur, A-E kapsamı değil.
