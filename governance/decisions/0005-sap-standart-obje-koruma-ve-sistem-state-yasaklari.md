...
**ZORUNLU akış:** Önce **released API** ara — released RAP BO (EML, `I_*TP`), released BAPI ya da released OData API (`API_*_SRV`; released RAP BO yoksa/operasyonu kapalıysa ve released BAPI yoksa, ya da uzak tüketimde — release edilmemiş BAPI'den ÖNCE); released RAP BO ancak canlı teyitleri geçerse (operasyon açık, alanlar yazılabilir) kullanılır. Yoksa SAP standart **BAPI** ara (`BAPI_*_CREATE`, `BAPI_*_CHANGE`), bulamazsan **RFC FM** ara (release edilmemiş resmi FM — RFC-enabled olmayan dahil; iç FM değil — tanım std/10 ADIM 4), bulamazsan **transaction (BDC)** ara, hâlâ bulamazsan kullanıcıdan manuel yapmasını iste. Asla direkt SQL yazma.
