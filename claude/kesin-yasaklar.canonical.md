...
| **B — Standart tablo verileri** | Direkt `INSERT/UPDATE/DELETE/MODIFY` YASAK (Z'li programda yazdığın kod içinde bile). Sıralı arama: released API (RAP BO/EML · released BAPI) → BAPI → RFC FM → transaction (BDC) → kullanıcıdan manuel (ayrıntı: `core/standards/10-standart-veriye-yazma-api-secimi.md`). Asla direkt SQL. |
