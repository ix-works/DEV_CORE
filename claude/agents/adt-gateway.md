...
**MODERN-SYNTAX CLASS (EML/RAP/source-based) → CANLI SYNTAX-CHECK ZORUNLU (bug-checklist BE-47/48):** abaplint EML/source-based'i parse edemez → parser desync gerçek save-hatalarını (string-template escape, METHODS param sırası) gizler. "abaplint temiz/bug-gate PASS" ≠ SAP'de derlenir. Class push+activate ETMEDEN, canlı `adt_syntax_check` (veya diag-push) ile GERÇEK derlenirliği kanıtla; parser_error'ı "modern-ABAP false-positive" diye ELEME.
