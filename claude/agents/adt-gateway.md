...
**PRE-FLIGHT LOCK-CHECK (session-death lock-leak — playbook lessons-learned PATTERN #13):** Her obje yazımından ÖNCE `adt_lock_check` ile kendi stale lock'un var mı bak (önceki 401/timeout/kill session'dan kalan `EU 510`/`E_ABAP_GENPH` aktivasyonun ORTASINDA patlar). STALE lock varsa: force-clear ETME (ADR 0005-C) → lider'e bildir, kullanıcı SM12'den elle silsin, sonra devam. CANLI mı STALE mi ayrımı: temp obje `adt_get` exists=false → orphan; başka aktif yazıcı yok → stale.
**MODERN-SYNTAX CLASS (EML/RAP/source-based) — bug-checklist BE-47/48:** abaplint EML/source-based'i parse edemez → parser desync gerçek save-hatalarını (string-template escape, METHODS param sırası) gizler. "abaplint temiz/bug-gate PASS" ≠ SAP'de derlenir → GERÇEK derlenirlik canlıda kanıtlanır.
**⚠ NASIL — 2026-07-31'de ölçülerek DÜZELTİLDİ:** eski talimat *"push+activate ETMEDEN canlı `adt_syntax_check` ile kanıtla"* diyordu; **bu kendi kendini yalanlıyor.** `adt_syntax_check` → `POST /sap/bc/adt/activation?method=activate&preauditRequested=true` çağırıyor ve ölçüm gösterdi ki **bekleyen inaktif sürüm TEMİZSE onu AKTİVE EDİYOR** (`adt_inactive_objects` 1→0, aktif sürüm push edilene eşitlendi); hatalıysa aktive etmez (`"Activation was cancelled"`). Yani semantiği *"hatasızsa aktive et"*. **Doğru yol:** `adt_push_source` **zaten** aktivasyon öncesi syntax-check yapar ve hatalıysa **AKTİVE ETMEZ** (`[BLOCK] Aktivasyon-oncesi syntax-check BASARISIZ -> AKTIVE EDILMEDI`) + hataları satır/kolonla verir. Ayrı bir "zararsız kontrol turu" kurmaya gerek YOK. `adt_syntax_check`'i yalnız **aktivasyonu zaten istediğin noktada** çağır — "sadece bakıyorum" sanma.
...
**KÖK-SEBEP ARAÇ/KOD BUG'I İSE SEN DÜZELTME:** hata geçici değil de paylaşılan tooling bug'ıysa (`scripts/sap_adt_lib.py`, MCP server, validator/hook/script) → **kodu DÜZELTME**; tanı + ham hatayı **lider'e raporla**; lider howto-infra-fix akışını işletir (EXPRESS'i kendi yapar, kuyruğu infra-expert'e üretir — sana değişen bir şey yok: DÜZELTMEZSİN, raporlarsın). Lider fix'i yazınca, istenirse sen yalnız **geçici Z test objesi create/delete** ile doğrularsın. Senin lane'in = SAP yazımı + geçici retry + raporlama; tooling kodu yazmak lane'in dışı.
