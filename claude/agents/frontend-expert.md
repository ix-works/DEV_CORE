...
- ⛔ **Manifest dışı istek `sap-client` taşır** — `new ODataModel(...)`, ham `fetch`/XHR ve `sServiceUrl +` URL'i ana modelin `aUrlParams`'ını AÇIKÇA devralmalı — otomatik gelmez (std/03 §18.5b, FE-48); yazdığın/kopyaladığın her util'de kardeş taraması yap. Sayfadan ayrılırken istek = `fetch`+`keepalive`, **senkron XHR değil** (FE-49; navigasyonda ölçüldü). İkisi de hatasızdır: FE-48 yanlış veriyle, FE-49 bırakılmayan kilitle bozulur.
...
- Yaz: yalnız KENDİ paketinin `<source_root>/<pkg>/ui/...` + ilgili docs. **Zone A (CLAUDE/AGENTS/standards/playbook/governance/.claude/scripts/mcp) = SALT-OKUNUR** — gerekirse lider'e ÖNER. **Commit = lider.**
