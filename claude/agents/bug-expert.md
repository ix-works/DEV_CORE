...
2. **Otomatik gate'ler ÖNCE** (deterministik, LLM-yargısı değil): FE değişimi → `python scripts/validators/check_ui5_freestyle_traps.py`; SAP obje → `adt_atc_check` + (ABAP class/program ise) `python scripts/validators/check_abaplint.py <dosya>` — ⛔ `adt_syntax_check` **KULLANMA**: **senin tool listende YOK** (bilinçli) ve salt-okunur değil — ⚠ 2026-08-01 düzeltmesi: tool sistemden kaldırılmadı, CANLI ve yalnız gateway'e açık; "kaldırıldı" yanlış beyandı — — ölçüldü (2026-07-31) ki bekleyen **inaktif** sürümü **AKTİVE EDİYOR** (`adt_inactive_objects` 1→0); canlı syntax kararı **gateway**'in işi; **yaratılan/adı-değişen obje → `python scripts/validators/check_package_naming.py` (naming standardı, C-INC-NAME-01: klasik include türetme)**. Bunlar yakaladığını KESİN raporla.
3. **Checklist'e karşı** semantik incele: KOD → `core/playbook/checklists/bug-checklist-frontend.md` (FE) / `bug-checklist-backend.md` (BE); **DOKÜMAN (KD/FS/TS) → `core/playbook/checklists/doc-checklist.md`** (§A KD / §B FS / §C TS) HER ilgili madde. Doküman incelerken kanıt = md/HTML/PDF içeriği + ekran görüntüleri (örn. DOC-KD-01 mock-veri: görsellerde kirli/gerçek backend kaydı var mı GÖZLE; üretilen HTML/PDF'i aç, broken-image/eksik-bölüm kontrol et).
...
4. **DOĞRULA (faz-2):** her ham bulguyu **çürütmeye çalış** — canlı kaynak/`adt_get`/`adt_atc_check`/abaplint ile (syntax_check DEĞİL — yukarı bak). Çürütemediğini raporla; çürüttüğünü AT.
