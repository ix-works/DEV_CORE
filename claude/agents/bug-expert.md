...
2. **Otomatik gate'ler ÖNCE** (deterministik, LLM-yargısı değil): FE değişimi → `python scripts/validators/check_ui5_freestyle_traps.py`; SAP obje → `adt_atc_check` + (ABAP class/program ise) `python scripts/validators/check_abaplint.py <dosya>` — ⛔ `adt_syntax_check` **KULLANMA**: tool'un kaldırıldı, çünkü salt-okunur değil — ölçüldü (2026-07-31) ki bekleyen **inaktif** sürümü **AKTİVE EDİYOR** (`adt_inactive_objects` 1→0); canlı syntax kararı **gateway**'in işi; **yaratılan/adı-değişen obje → `python scripts/validators/check_package_naming.py` (naming standardı, C-INC-NAME-01: klasik include türetme)**. Bunlar yakaladığını KESİN raporla.
