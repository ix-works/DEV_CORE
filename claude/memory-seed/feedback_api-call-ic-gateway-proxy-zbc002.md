---
name: feedback_api-call-ic-gateway-proxy-zbc002
description: "RFC-dest kullanan TÜM API call'larda SM59 yerine ZBC002 helper + /iwfnd/cl_sutil_client_proxy iç gateway mimarisi (proje standardı)"
metadata: 
  node_type: memory
  type: feedback
...
- **Token+URL:** paylaşılan `ZBC002_CL_GET_TOKEN` (sahibi D_OSOZEN, BC paketi). `get_token(iv_method)` → CSRF; `get_host(iv_method, iv_token)` → tam URL. **Bu sınıfa DOKUNMA — sadece kullan** (başka geliştiricinin shared objesi).
...
- **POST'u kendi paketin altında** yaz (ZBC002'ye genel POST eklenmez); response body parse için `cl_abap_conv_codepage=>create_in( )->convert( ev_response_body )`. Çalışan tam örnek: `ZQM012_CL_GET_TOKEN` (get_token + save_userdecision).
