...
- ✅ **MCP `adt_get object_type='func'` ÇALIŞIR (2026-09-13, Q261):** fonksiyon grubunu arama indeksinden kendisi çözer (quickSearch `objectType=FUGR/FF` + tam ad) → `exists:true` + `resolved_uri` + `function_group` + kaynak + metadata. Var olmayan ad → `exists:false` + `probe` (hangi aramanın koşulduğu); arama/okuma hatası → `ok:false` ("yok" DEĞİL). `adt_where_used(func)` aynı çözümlemeyi kullanır: yok → `OBJECT_NOT_FOUND`, var ama çağıransız → `count:0` + `existence_verified:true`. Tarihçe: 2026-09-04'e kadar mevcut FM'e `exists:false`, sonra `ok:false error:"unexpected"` dönüyordu. ⚠ `adt_lock_check` / `adt_atc_check` func için kanal HÂLÂ YOK. ⚠ Elle arama yaparken `objectType=FUNC` KULLANMA — var olan FM için 0 döner; `FUGR/FF` ya da filtresiz tam ad kullan. ⚠ Çıplak FM ucunu elle GET'lerken Accept `application/vnd.sap.adt.functions.fmodules.v3+xml` ver (`core.v1`/`application/xml` → 406). **KAPSAM:** bu sorun YALNIZ `object_type='func'`a özgüdür (FM group-resolution); genel `adt_get` DDIC-okuması güvenilirdir (DTEL/DOMA/TABL/struct/ttyp KÖK-FIX 2026-06-16, hafıza `feedback_adt-get-ddic-read-fixed`) — "adt_get genelde güvenilmez" algısı YARATMA.
