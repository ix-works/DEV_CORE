...
| **classrun app-server load-cache** | classrun bir class'ı bir kez yükleyince, push+activate sonrası bile **eski load'u** çalıştırabilir (delete+recreate'e rağmen). → iterasyonda **yeni class adı** kullan ya da bu kanaldan değil FM/SOAP-RFC'den git. ⚠ **2026-07-30 DÜZELTMESİ — "yeni class adı" çaresi YETMEYEBİLİR:** hiç yüklenmemiş **iki ayrı taze sınıf** (`…_SMOKE_TMP`, `…_DUMAN2_TMP`) arka arkaya yaratıldı, ikisi de **aktive oldu**, kaynaklarında `INTERFACES if_oo_adt_classrun` + `METHOD …~main` **doğrulandı** — yine de ikisinde de sahte *"does not implement"*. **Elenenler (ölçüldü):** CSRF regresyonu (`test_csrf_header_injection` **5/5 geçiyor**) · bare-header tuzağı (`run_classrun` zaten `_get_headers`+`_request_with_csrf_retry` kullanıyor) · sınıfın kendisi (aktif + arayüz kaynakta). **Kalan hipotez:** sistem **çok app-server'lı** ve ADT bağlamı tek sunucuya çiviliyor (aynı gün ölçülen contextid: `SID:ANON:<host>_<SID>_00`) → sınıf A'da aktive olup classrun B'ye düşünce load yok. Taze isim bunu **çözmez**, çünkü yeni sınıf da aynı asimetriyi yaşar. → **Doğru refleks: kanal değiştir** (Eclipse **F9** / SOAP-RFC), "bir daha dene" değil. *Sinyal: sınıf yapısal olarak geçerliyken 2. denemede de aynı mesaj geliyorsa, sınıfla uğraşmayı bırak.* |
