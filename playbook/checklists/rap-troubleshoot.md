...
| **DELETE validation'ı hiç çalışmıyor — silme her zaman geçiyor** (BE-60, 2026-07-28) | Gövdede **`READ ENTITIES`** var. Delete-validation koştuğunda örnek geçiş tamponunda **ARTIK SİLİNMİŞTİR** → READ **boş** döner → `IF lt IS INITIAL. RETURN.` (ya da boş `lt` üzerinde `LOOP`) → kontrol **hiç koşmaz**. Hata YOK, uyarı YOK. ATC/abaplint/`adt_syntax_check`/`run_review` **hiçbiri görmez** | ✅ Doğrusu: **doğrudan `keys` üzerinde** çalış — anahtarlar zaten girdide. `keys`'ten yalnız **entity key** alanları okunabilir (başka alan lazımsa DB'den çek). Canlı vakada 10 delete-validation'ın **9'u** bu hatayı taşıyordu, 1'i doğruydu → **repodaki doğru örneği bul ve ona hizala**. Ayrı hata sınıfı: `delete;` açık olup **hiç** delete-validation'ı olmayan entity — onu da tara |
| **`field a, b; create; update;` yazdım ama alan payload'da yokken de tetikleniyor** | `create;`/`update;` **OPERASYON** tetikleyicisidir; `field …` listesi tetikleyiciyi **daraltmaz**. FE'den alanı çıkarmak validation'ı susturmaz | Muafiyet gerekiyorsa **behavior implementation içinde** ver (kalem bazlı `CONTINUE`), FE'de değil |
