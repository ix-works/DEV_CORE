...
| **C-CDS-FROM-05** | ⛔ **Classic DDIC view** (`sqlViewName`'li) ise: `FROM`/`JOIN`'de **replacement tablosu** (`DD02L-VIEWREF` dolu — `MSEG`/`MKPF`/`MBEW`/`MSSA`…) var mı? Varsa **`nsdm_e_*` karşılığını kullan**; yoksa view hatasız aktive olur ve **DAİMA 0 satır** döner. *(view entity'de sorun yok; ama view entity DB view üretmez → AMDP `USING` zinciri varsa çözüm o değildir)* | `manual:nsdm-replacement-check` (oto-gate YOK — ADR 0019 §4 merdiven, bilinçli) | BLOCKER | `../adt-cds.md` **CDS-NSDM-01** · `../lessons-learned.md` PATTERN #21 |
