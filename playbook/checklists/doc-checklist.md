...
| DOC-FS-06a | **§1.1 versiyon satırı KISA — ÖLÇÜLEBİLİR, gate'li** — satır başına 1-2 satır "ne değişti" (madde listesi, madde/§ atfı); "neden/nasıl bulundu" anlatısı EK "Karar ve Kanıt Günlüğü"ne. Ölçüt MEKANİKTİR: §1.1 (ve başlıksız versiyon tablosu / kapak / altbilgi) satırı **≤ 400 karakter** (`VERSION_ROW_MAX`). GATE (warn-first/**advisory** — sayar ve listeler, commit'i DURDURMAZ; bulguda exit 1 yalnız `post_validate` hook'unun `--bulguda-exit1`'iyle): `check_fs_no_analysis_log.py`. | MEDIUM (EKSİK) | std §2.0 İLKE-2b · §2.2 B1.1 |
| DOC-FS-06b | **11-B birikmemiş + yayılım tablosu tam — REVIEWER YARGISI (advisory, otomatik gate YOK)** — §11-B'de yalnız AÇIK (ya da bloke etmeyen) kararlar; kapanan karar gövdeye SONUÇ olarak işlenmiş, satırı EK "Karar ve Kanıt Günlüğü"ne inmiş (karar/kim/ne zaman/gerekçe/kanıt atfı; reddedilen seçenek "neden dışlandı" ile). ⭐ **YAYILIM TABLOSU (2026-08-19):** her karar kaydı **dokunulacak yerlerin listesini** taşır ve liste tamamlanmadan karar **kapanmış sayılmaz** — karar N yerde yaşar, düzeltme turu N-1'ini yapar. ⛔ **Boş/eksik yayılım tablosu ⇒ kayıt GEÇERSİZ** (tablo koyup boş bırakmak, tablo koymamaktan **daha kötüdür**: koruma sanısı üretir). ⚠ Bu yarım MEKANİKLEŞTİRİLEMEZ ("karar kapandı mı", "liste tam mı" = anlam yargısı) ⇒ arkasında script YOKTUR, reviewer bakar. | MEDIUM (EKSİK) | std §2.0 İLKE-2b · §2.2 B11-B |
...
| DOC-TS-05 | **Mesaj envanteri TAM (§10.1)** — geliştirmenin üreteceği **HER** mesaj listelenmiş mi (**envanter**, örnek tablo değil); her satırda **metin birebir** + **numaralı yer tutucu** (`&1..&4` — çıplak `&` DEĞİL) + **her yer tutucunun anlamı** + **üretim noktası** (sınıf/metot · RAP validation/determination) + **kullanıcı aksiyonu** (`E`/`A`'da zorunlu) var mı. Metinler **≤ 73 karakter** mi (T100-`TEXT` = `CHAR 73`; `populate_message_class.py` fail-closed durur — aşan metni SAP **sessizce KIRPAR**, ekranda yarım cümle kalır). Metinleri **kullanıcı** vermiş mi (ADR 0005-D — AI önermez/uydurmaz). ⛔ **Build sırasında doğan mesaj TS'e GERİ İŞLENMİŞ** mi (TS revizyonu — *"sonra ekleriz"* YOK)? Uzun metin (long-text) isteğe bağlı; yazıldıysa *"SE91 gerektirir"* şerhi var mı (programatik yol YOK, `mc:documented="false"` sabit). *(2026-08-24 ve 2026-08-29: aynı pakette build ortasında doğan mesajlar iki kez bloke eden onay turu açtı.)* ⚠ Otomatik gate YOK — reviewer yargısı. | HIGH (EKSİK) | std §3.2 B10.1 · ADR 0005-D · [`adt-message-class.md`](../adt-message-class.md) §27 |
