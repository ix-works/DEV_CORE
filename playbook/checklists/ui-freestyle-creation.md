...
| UI-SAVE-05 | Belge kilidi varsa: unload bırakma = `fetch` + `keepalive` + CSRF + client parametresi (senkron XHR sayfa kapanırken GİTMEZ); kilit bırakılan her çıkış yolunda (geri/kayıt/silme) kilit bayrağı sıfırlanır | BLOCKER | howto-document-lock §4 · FE-49 |
