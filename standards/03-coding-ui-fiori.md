...
- Yeni app → `ui/<app>/` altına; app'in package.json'u **ince**: name + scripts + **`devDependencies`'te tooling paketlerinin ADLARI** — en az `ui5*.yaml`'da custom middleware/task olarak kullanılan her paket (`@sap/ux-ui5-tooling` → `fiori-tools-proxy`/`-preview`/`-appreload`; `@sap-ux/ui5-middleware-fe-mockserver` → `sap-fe-mockserver`). Sürüm aralığı §2.1 ortak setle aynı yazılır; paketler **kökte hoist** edilir, app'e ayrı kurulum GELMEZ (§14 checklist'in "`devDependencies` → `@sap/ux-ui5-tooling` dahil" satırı da bunu ister).
  ⚠ **Neden ADLAR şart (Issue #298, ölçüldü):** UI5 CLI 4 custom middleware'i **app'in kendi `package.json` bağımlılıklarından** çözer; kökte hoist edilmiş `node_modules` tek başına yetmez ⇒ adlar yoksa `start-noflp`/`start-mock` `Could not find custom middleware fiori-tools-proxy` ile AÇILMAZ (UI5 CLI 4.0.57 ve 4.0.69; kontrol grubu: aynı app'e yalnız adlar eklenince açıldı). **Sınır:** UI5 CLI 3.x · build/deploy script'leri · adları ekledikten sonra kökte yeniden `npm install` gerekip gerekmediği **ÖLÇÜLMEDİ** — şüphede `npm install`'ı KÖKTE koş.
- **`npm install`'ı `ui/` KÖKÜNDE çalıştır, app dizininde DEĞİL.** Tooling tek `ui/node_modules`'a hoist olur; yeni app **~anında** katılır (8dk install yok). `cd <app> && npm install` = gereksiz per-app node_modules → YAPMA. **GATE:** `scripts/hooks/pre_tool_guard.py` (PreToolUse Bash) app-içi `npm install/ci/add`'i bloklar. Lokal çalıştırmak için app dizininde kurulum GEREKMEZ (kök kurulumu yeter) — app dizininden `npm run start-noflp`/`start-mock` (bin ata-dizin `ui/node_modules/.bin`'den çözülür). **Ön koşul:** app `package.json`'unda yukarıdaki tooling ADLARI beyanlı (UI5 CLI 4'te bin'in bulunması middleware'in bulunması demek DEĞİLDİR).
...
| `useBatch: false` | OData V2 SEGW servisleri için. ⚠ **İSTİSNA:** dosya/base64/uzun metin taşıyan bir **function import** varsa o çağrı `$batch` ister — manifest DEĞİŞMEZ, ikinci model açılır (§18.5a) |
...
sap.ui.define([
  ...
  return Controller.extend("com.mycompany.myapp.controller.List", {
    ...
    // ⚠ V2'de urlParameters URL'e serileşir → BÜYÜK yük (dosya/base64) burada 414 verir. §18.5a
...
### 18.5a Function import ile BÜYÜK YÜK — ölçülmüş HTTP 414 duvarı (`useBatch:false` istisnası)

> `prior-art: kısmen bulundu` — tüketen projede ölçümsüz tek risk şerhi vardı (2026-09-05, ölçüm
> ön koşulu hiç sağlanmamış, register'a da düşmemiş); **core'da 414 kaydı YOKTU** (grep: 0).

**Sınıf.** OData **V2** `callFunction`, `urlParameters`'ı **URL'e** serileştirir (`sap/ui/model/odata/v2/
ODataModel.js`; gövde ancak 5. argümanla doğar, `undefined` bırakılırsa istek **gövdesizdir**). Dosya /
base64 / uzun metin taşıyan bir function import bu yüzden **istek satırını** şişirir ve ters proxy
duvarına çarpar: **HTTP 414 Request-URI Too Long**. Kusur RAP/Gateway'de değil, **taşıma katmanındadır**.

**ÖLÇÜLDÜ** (`s4_private`, 2026-09-16; ölçüm ortamı tek sistem — eşik SAYISI kuruluma bağlıdır, **SINIF** bağlı değildir):

| Ölçüm | Değer |
|---|---|
| Geçen en büyük istek satırı | **8.398 B** |
| 414 veren en küçük istek satırı | **8.498 B** |
| Duvarın **sahibi** | **SAP Web Dispatcher** — 414 gövdesindeki `<!-- This error page was generated by SAP Web Dispatcher (-)! -->` ile kanıtlandı (**ICM değil**) |
| İlgili parametre | `wdisp/max_permitted_uri_len`. ⛔ `icm/HTTP/max_request_size_KB` **GÖVDE** sınırıdır (~100 MB) — bu kusurla **ilgisizdir**, oraya bakmak tur kaybettirir |
| `$batch` ile ölçülen tavan | ≥ **512 KB** iç istek satırı (~**60×**) |

⛔ **Duvarı yükseltme yoluna GİTME:** (a) URL'ler access log'a düşer ⇒ dosya içeriği log'a yazılır
(b) yönetilen bulutta (RISE/PCE) Web Dispatcher parametresi müşteride değildir, ticket ister
(c) sınır kalkmaz, yalnız ötelenir — aynı kusur daha büyük dosyada geri gelir.

**ÇÖZÜM — o TEK function import için ikinci model (`useBatch:true`); `manifest.json` DEĞİŞMEZ.**
`$batch`'te dış URL sabittir (`…/$batch`), yük **gövdeye** iner.

```javascript
// Yalnız büyük-yük taşıyan function import için, TEMBEL yaratılır.
// manifest'teki varsayılan model useBatch:false KALIR (§3.2) — bu bir İSTİSNADIR, kural değişikliği değil.
_getBatchModel: function () {
  if (!this._oBatchModel) {
    ...
    this._oBatchModel = new ODataModel(this._secondModelUrl(oMain.sServiceUrl, oMain), {
      ...
      defaultBindingMode: "TwoWay",
      defaultCountMode:   "Inline",
      // ⛔ ZORUNLU: ana modelin metadata anahtarını DEVRAL — literal dil yazma.
      metadataUrlParams:  oMain.mMetadataUrlParams
    });
  }
...
⛔ **`metadataUrlParams` DEVRİ SEÇİMLİK DEĞİL — bir ASILMA yolunu kapatır.**

`ODataModel` iki AYRI havuz kullanır (`ODataModel.js:427-428`): `oServiceData` (CSRF token) **`sServiceUrl`**
başına paylaşılır, `oMetadata` ise **`sMetadataUrl`** başına. manifest'ten kurulan V2 modele UI5'in kendisi
`mMetadataUrlParams['sap-language']` ekler (`Component.js:1909-1923`); doğrudan `new ODataModel(...)` ile kurulan
model bunu **almaz** ⇒ anahtarlar **zorunlu olarak ayrışır** ⇒ ikinci modelin **kendi `$metadata` GET'i** doğar.

Ayrışmanın asıl bedeli o ek istek değil, **başarısız olma ihtimalidir**:

1. `ODataMetadata.js:87-95` — yükleme hata alırsa `loaded()` **hıç settle olmayan** bir promise döndürür
   (yalnız sonraki **başarılı** bir yükleme `fnResolve`'u çağırırsa çözülür).
2. `ODataModel.js:5095` — `_processRequest` her isteği `oMetadata.loaded().then(...)`e asar; **`.catch` YOK, `timeout` YOK**.
3. ⇒ istek **hiç gitmez**, `success`/`error` **hiç koşmaz** ⇒ kendi `busy` bayrağını `false`'a çeken kod da koşmaz
   ⇒ sayfa ömrü boyunca **kalıcı meşgul**; çıkış F5.

Ana modelin anahtarı devralınırsa `ODataModel.js:446-449` onun **zaten çözülmüş** `ODataMetadata`'sını yeniden
kullanır ⇒ **ek istek de bu yol da yok olur**. `mMetadataUrlParams` model örneğinde durur (`:361`) ⇒
`metadataUrlParams: oMain.mMetadataUrlParams` **dil literali yazmadan** aynı anahtarı üretir; yeni bir karar değildir.

⚠ Devralmayacaksan (ör. bilerek farklı anahtar), `busy` bayrağını bir **zaman aşımıyla** kurtar ve kullanıcıya söyle.
⛔ Bu maliyet **ağ izinden** ölçülür: aynı vakada *"metadata paylaşılıyor, sorun yok"* hükmü **kaynak okumasıyla**
verildi ve **canlı ölçüm çürüttü** — paylaşılan `oServiceData`'ydı, `oMetadata` değil.

⛔ **DENENDİ, ÇALIŞMIYOR — tekrar deneme:** parametreleri `application/x-www-form-urlencoded`
**gövdeyle** göndermek. Gateway **HTTP 200** döner ama gövdeyi **sessizce yok sayar**; parametre boş
gelir, motor "zorunlu alan boş" hatası verir. **200 gördün diye geçme — gövdeyi aç.**

**Ham `$batch` gövdesi** (elle kurulursa; satır sonları **CRLF**):

```text
--batch_ID CRLF
Content-Type: multipart/mixed; boundary=changeset_ID CRLF
CRLF
--changeset_ID CRLF
Content-Type: application/http CRLF
Content-Transfer-Encoding: binary CRLF
CRLF
POST FuncImport?Param='<urlencoded>' HTTP/1.1 CRLF
Content-Type: application/json CRLF
Accept: application/json CRLF
Content-Length: 0 CRLF
CRLF
CRLF            <-- ⛔ BU BOŞ SATIR ZORUNLU (iç isteğin gövde sonu)
--changeset_ID-- CRLF
CRLF
--batch_ID-- CRLF
```

⛔ Kapanış ayracından önceki **boş satır** yoksa Gateway `HTTP 400 — malformed syntax` verir ve mesaj
**boyutu işaret etmez**. Üç deneme bu yüzden *"batch desteklenmiyor"* diye yanlış yorumlanmaya çok
yaklaştı; **kontrol grubu** (165 B'lık minik batch) da 400 verince sorunun **biçim** olduğu, **boyut**
olmadığı anlaşıldı (PATTERN #19 — ② ÖLÇ'ün özü).

**Ön yüzde sınır koy.** `FileUploader`'ın `maximumFileSize`'ı, kodda kullanılan boyut sabitiyle **aynı
kaynaktan** beslenir (iki ayrı literal = sessiz ayrışma); istek hiç doğmadan reddedilir.
